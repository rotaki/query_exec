    ...
    if opt.bench_all {
        ...
        for query_id in 1..=22 {
            ...
            for _ in 0..opt.warmups {
                let exec =
                    query_executor.to_executable::<PipelineQueue<InMemStorage>>(physical.clone());
        ...
        let file_name = format!("tpch_pipeline_results_inmem_sf_{}.csv", opt.scale_factor);
        ...
        let start = std::time::Instant::now();
        let exec = query_executor.to_executable::<PipelineQueue<InMemStorage>>(physical);
